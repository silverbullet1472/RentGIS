...
def lease_post_extract(post_url):
    ...
    try:
        ...
        post_value_list = info_parser.lease_parser(post_r)
...
def get_lease(city, db_code):
    # 创建该城市数据表
    db_option.create_table_lease(db_code)
    # 进入城市选择页面
    ...
    print("获取到此城市链接:" + city)
    # 进入城市一级区域链接
    city_r = xun_request(city_href + '/zufang/')
    ...
    for first in first_list:
        ...
        for second in second_list:
            ...
            # 二级页面首页中获取此区域总页数
            total_page = second_index_bs.find('.pageBox a span')[-2].get_text(strip=True)
            print("此二级区域总页数:" + total_page)
            # 拼接得到此二级区域所有页面url
            post_page_list = post_page_list + [second[0] + "pn" + str(x) for x in range(1, total_page + 1)]
        ...
        post_collect_pool = Pool()
        # 获得到所有url并组合成table(table中url_list中含重复url)
        post_url_table_all = post_collect_pool.map(func=lease_post_collect, iterable=post_page_list)
        # 对table进行去重
        ...
        post_value_table = post_extract_pool.map(func=lease_post_extract, iterable=post_url_set)
